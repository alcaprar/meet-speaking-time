...
const intervalToJoinFunc = function (tab) {
  chrome.tabs.sendMessage(tab.id, { join: true }, function (resJoin) {
    if (resJoin && resJoin.status) {
      clearInterval(intervalToJoin);
      intervalSetEnvironment = setInterval(
        intervalSetEnvironmentFunc.bind(null, tab),
        500,
      );
    }
  });
...
const intervalSetEnvironmentFunc = function (tab) {
